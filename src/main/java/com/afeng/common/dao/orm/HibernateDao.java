...
/**
 * 
 * @Description : 封装SpringSide扩展功能的Hibernate DAO泛型基类.扩展功能包括分页查询,按属性过滤条件列表查询. 可在Service层直接使用,也可以扩展泛型DAO子类使用,见两个构造函数的注释.
 * @param <T>   : DAO操作的对象类型
 * @param <PK>  : 主键类型
 * @Date        : 2015-7-7
 * @author      : afeng  
	...
	protected Criterion buildCriterion(final String propertyName,
		...
		case LIKE:
			...
			if ((ESCAPE.toString()).equals(value)) {
				...
				criterion = new com.afeng.common.dao.orm.LikeExpression(propertyName,
